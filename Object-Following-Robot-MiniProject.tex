...
Anupam Dutta (Roll No. 34900323003)\par
...
\medskip
When we first built the prototype, we included a 0.96-inch SSD1306 OLED display and an SG90 micro servo motor. However, when the servo motor turned, it drew too much current and caused the voltage on the 5V line to drop. This voltage drop caused the Arduino to reset repeatedly and made the sensor readings unstable. To solve this power issue and make the robot run reliably on a single battery pack, we removed the display and the servo motor from the final design.
\clearpage
