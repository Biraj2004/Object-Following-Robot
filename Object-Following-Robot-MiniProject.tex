...
Connecting DC motors and microcontrollers to the same power supply can cause issues. When DC motors start up or change directions, they draw a lot of current. This can drop the battery voltage and reset the Arduino UNO. To prevent this, we split the power lines. The 7.4V battery pack connects directly to the L298N motor driver to power the motors. The driver's internal regulator steps down the voltage to 5V to power the Arduino UNO. We also connected the grounds of the battery, Arduino, and motor driver together to establish a shared ground reference.\par
When we first built the prototype, we included a 0.96-inch SSD1306 OLED display and an SG90 micro servo motor. However, this configuration caused transient voltage drops and microcontroller resets under load, leading to their removal from the final design.
\clearpage
