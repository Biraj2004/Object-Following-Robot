...
Bikkram Das (Roll No. 34900323012)\par
...
\chapter*{Abstract}
\vspace{0.2in}
This mini project presents the design and construction of an autonomous Object Following Robot using the Arduino UNO microcontroller. The robot is designed to detect and follow a target object or person in real time while keeping a safe distance. The sensing unit consists of an HC-SR04 ultrasonic distance sensor mounted in a fixed position on the front of the robot chassis, which measures the distance to the target. To help with distance tracking, two infrared (IR) proximity sensors are mounted on the left and right sides of the chassis. These IR sensors detect if the target moves sideways, allowing the robot to turn and follow the target dynamically.\par
\medskip
The robot is driven by four battery-operated (BO) geared motors controlled by an L298N dual H-bridge motor driver. The Arduino UNO reads the sensor signals, calculates the distance and direction of the target, and controls the motor speed using Pulse Width Modulation (PWM). If the target goes out of range, the robot stops and waits for it to reappear. The system is powered by a 7.4V battery pack made of two 18650 Lithium-Ion cells with a shared common ground. We tested the assembled prototype extensively. The results show that the robot follows targets reliably between 10 cm and 35 cm, backs up to recover a safe distance if the target gets closer than 10 cm, and turns smoothly to track lateral movements.\par
\clearpage
...
The pins on the Arduino UNO are mapped to organize connections. Table \ref{tab:pin_mapping} lists the physical pin connections, and Figure \ref{fig:pin_config} illustrates the corresponding visual schematic layout.
...
\vspace{0.1in}
\begin{center}
\includegraphics[width=0.85\textwidth,keepaspectratio=true]{images/pin configuration.png}
\captionof{figure}{Arduino UNO and Sensor/Driver Pin Configuration Schematic}
...
Connecting DC motors and microcontrollers to the same power supply can cause issues. When DC motors start up or change directions, they draw a lot of current. This can drop the battery voltage and reset the Arduino UNO. To prevent this, we split the power lines. The 7.4V battery pack connects directly to the L298N motor driver to power the motors. The driver's internal regulator steps down the voltage to 5V to power the Arduino UNO. We also connected the grounds of the battery, Arduino, and motor driver together to establish a shared ground reference.
\clearpage
...
Testing began by calibrating the ultrasonic and infrared sensors. The ultrasonic sensor was tested against a flat target at distances from 5 cm to 100 cm to verify ranging accuracy. The infrared sensors were calibrated using their onboard potentiometers to trigger when an object was within 8 cm, preventing false detections from ambient light. Table \ref{tab:calibration} outlines the sensor calibration data and command mappings. Additionally, trials were conducted on different floor surfaces to calibrate the minimum PWM start-up thresholds for the BO geared motors, ensuring smooth motion initiation. We also verified the ultrasonic sensor's performance against both hard and soft target surfaces to ensure consistent echo reception. Figure \ref{fig:circuit_diagram} shows the circuit diagram, Figure \ref{fig:assem_robot} shows another perspective of the physical prototype, and Figure \ref{fig:wiring_inside} shows the internal chassis wiring.
...
\vspace{0.1in}
\begin{center}
\includegraphics[width=0.75\textwidth,keepaspectratio=true]{images/4WD Car.png}
...
Although we removed the display and the servo motor from the final prototype due to power and voltage issues, future versions can add them back by improving the circuit:
\begin{enumerate}[noitemsep,topsep=2pt,parsep=0pt,partopsep=0pt]
    \item Re-integrating the SG90 micro servo motor to rotate the ultrasonic sensor, allowing it to scan the environment up to 180 degrees.
    \item Re-introducing the 0.96-inch SSD1306 OLED display to show the target distance and robot actions in real time.
    \item Adding an LM2596 buck converter to isolate the servo's power draw from the microcontroller and prevent resets.
    \item Upgrading the microcontroller to an ESP32 for wireless control and camera-based human tracking (ESP32-CAM).
    \item Implementing a PID control loop to make the robot accelerate and decelerate more smoothly.
\end{enumerate}
\clearpage
